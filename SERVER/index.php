...
<head>
    ...
    <!-- Importing Google Fonts: Plus Jakarta Sans for UI, Syne & JetBrains Mono for a striking logo/branding look -->
    ...
    <style>
        ...
        /* Subtle reflection gloss on logo */
        .brand-icon::after {
            content: '';
            position: absolute;
            top: 0;
...
<body>
    ...
    <script>
        ...
        // Helper function to format IP address with highlighted host bits
        ...
        // Helper to convert an IP string into a comparable integer for strict Ascending sorting
        function ipToNum(ip) {
            return ip.split('/')[0].trim().split('.').reduce((acc, octet) => ((acc << 8) + parseInt(octet, 10)), 0) >>> 0;
        }

        // SVG asset generator for Internet / Network Globe
        ...
        // SVG asset generator for USB Flash Drive / Port
        ...
        function updateDashboard(workstations) {
            ...
            // Explicitly sort workstations in strict ascending numerical order by IP address
        ...
        function filterWorkstations() {
            ...
            const filtered = cachedWorkstations.filter(ws => {
                ...
                if (selectedFilter === 'online') {
                    ...
                    matchesCategory = (ws.usb_status !== 'no-usb');
                }
